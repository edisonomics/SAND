...
% set up group of compounds as random permutation
npeak=size(tabsumm_refine2_sele,1);
nele=floor((npeak-1)/(ncompound-1));
reparray=repelem(nele,ncompound-2);
group_ind_nodss=repelem(1:(ncompound-1),[reparray npeak-1-sum(reparray)]);
peak_group_vec=[group_ind_nodss(randperm(length(group_ind_nodss))) ncompound];
% random A for each peak
amax=max(tabsumm_refine2_sele(:,3));
conc_range=[amax/2 amax];
conc_vec=rand([npeak-1 1])*(conc_range(2)-conc_range(1))+conc_range(1);
conc_vec=[conc_vec; conc_range(2)];
tabsumm_refine2_sele(:,3)=conc_vec;
...
% relative concentration matrix for each compound
rel_conc_rang=[0.2 1];
rel_compd_conc_mat=rand([nsample ncompound-1])*(rel_conc_rang(2)-rel_conc_rang(1))+rel_conc_rang(1);
rel_compd_conc_mat=[rel_compd_conc_mat repmat(rel_conc_rang(2),[nsample,1])];
%
sigma=1e-4;
% simulate spectra
fid_mat=[];
