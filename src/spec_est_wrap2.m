function [para perf]=spec_est_wrap2(resvec,timevec,sampind,para_add_list)
...
% Yue Wu 06/23/2020
% OS 09/03/2022 revised part of it due to warnings
...
while true
  ...
  [para_est perf_train]=spec_optimize2(resvec(sampind.trainind),timevec(sampind.trainind),para_initial_array,para_iniguess_rang_comb,para_add_list);
  ...
  if (k>1)&(perf_valid>(1-para_add_list.imp_ratio)*perf_last)
    fprintf('adding new signal does not improve validation performance \n');
    if ~para_add_list.train_complete
      break;
    end
  end
  if nsig>0&k>=nsig
    fprintf('maximal number of signal is arrived \n');
...
[para_est perf_train]=spec_optimize2(resvec(sampind.trainind),timevec(sampind.trainind),paraarray_last,para_range_last,para_add_list_retrain);
